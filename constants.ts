...
*   **明确场景和效果**：把用户需求和框架设计编写进说明文档。

### 实践案例

**1. 同步前期调研结果**
- 需求收集：基于材料明确真实工作流 + 咨询专家了解真实痛点 + 明确AI功能边界。
- 架构设计：筛选必要功能 + 设计大致框架 + 定义输出要求。
...
- 开发人员专注于核心功能，不被流程打扰。`,
  ...
  },
  {
    ...
    content: `### 底层逻辑
开发人员输出代码和Test Sample→ PM基于产品功能+业务逻辑打分 → 反馈给开发人员做进一步定性Evals → 优化方向判断是否执行并技术探针看能否有资源执行 →汇总给开发迭代优化功能。
![快速验证，双向翻译](../assets/双向翻译.png)
...
>用法：可以copilot读取本readme，直接拆分成对应章节和模板
...
- 必备条目：流程概览图、每步输入/输出、负责人、依赖资源、自动化化程度（手工/半自动/全自动）、AI功能。
- 推荐结构：

| 章节 | 工作流与输入 | 痛点 | AI功能 |
|------|-----------|-------|--------|
| {第一章} | {PTR的附录A} | {数据摘取复杂，对比需人工操作} | {自动化对比流程} |


### CH03 — 架构设计 (03_架构设计)

- 目的：定义系统分层（如：Hand/Brain/UI/用户），并说明每层的输入/输出格式和流程。
- 必备条目：总体架构图、各层职责表、核心接口规范（输入/输出 schema）。
- 推荐结构：
	1. 总体架构与图示
	2. 各层职责一览（表格）
...
- 目的：把功能与合规的指标量化，给开发、评审与验收提供可测的 KPI 和验证方法。
- 必备条目：人工标准的输出样式、质量指标（准确率/引用准确率）、CMDE类合规指标。
- 推荐结构：
	1. 人工标准的输出样式
	2. 定性/定量指标（如准确率、生成速度等）
	3. 各阶段Quality Gate清单
	
- 示例字段：总周期由 10 天降至 1-3 天；差异判断假阴率=0%。
...
- 推荐结构：
	...
	2. 技术选型建议（工具/库/平台）
	3. 采纳决策流程
- 示例字段：DeepSeek-OCR优化LLM输入解析→ 提高解析精度和速度，但不适用于PoC小规模输入；优先级：中。

### CH07 — 总结复盘 (07_总结复盘)

- 目的：产出可复用的经验、Lessons Learned 与可迁移的 Skill 包。
- 必备条目：完成情况回顾、成功要点、失败/风险及原因、可复用资产列表（模板/Prompt/脚本）、后续建议。
- 推荐结构：
	1. 概要回顾（目标 vs 实际）
	2. 成功要点与关键决策
	3. 问题与改进建议
	4. 可复用资产（Skill 包索引）
- 示例字段：会议三段式模板、PM 一键包、Brain 层 Prompt。
- 复用检查清单：是否列出可复用的具体文件与路径？是否有短句的“复制使用指南”？

### CH08 — Skill 包 (08_Skill包)

- 目的：把重复可复用的方法、Prompt、会议模板等以包的形式沉淀，便于跨项目复用。
- 必备条目：Skill 描述、适用场景、输入/输出、最小示例、维护者。
- 推荐结构：
	1. Skill 简介（一句话）
 2. 适用场景与限制
 3. 输入/输出示例
 4. 使用步骤（复制→填充→运行）
 5. 维护与版本记录
- 示例字段：会议三段式 SOP、PM一键包。
- 复用检查清单：是否包含最小可执行示例？是否标注维护者与版本？`
    },
    author: 'Jianan Huang',
    authorAvatar: AUTHOR_AVATAR
  },
